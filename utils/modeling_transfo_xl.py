...
class RelPartialLearnableMultiHeadAttn(nn.Module):
    ...
    def trams(
        ...
        qlen = attn_mask.size(0)
        stdlen = qlen + remain_mem_num

        mem_QKk, std_QKk = QKk[:-stdlen], QKk[-stdlen:]
        mem_k, std_k = w_head_k[:-stdlen], w_head_k[-stdlen:]
        mem_v, std_v = w_head_v[:-stdlen], w_head_v[-stdlen:]

        mem_QKk_combined = (mem_QKk.norm(dim=-1) * torch.sum(mem_QKk, dim=-1).abs())

        _, topk_indices = mem_QKk_combined.topk(k=topk_num, dim=0, largest=True)
        sorted_indices = topk_indices.sort(dim=0).values

        expanded_indices = sorted_indices[:, :, :, None]

        QKk = torch.cat([torch.gather(mem_QKk, dim=0, index=expanded_indices.expand(-1, -1, -1, self.d_model)), std_QKk], dim=0)
        w_head_k = torch.cat([torch.gather(mem_k, dim=0, index=expanded_indices.expand(-1, -1, -1, self.d_head)), std_k], dim=0)
        w_head_v = torch.cat([torch.gather(mem_v, dim=0, index=expanded_indices.expand(-1, -1, -1, self.d_head)), std_v], dim=0)

        r_head_k = r_head_k[-stdlen-topk_num:]
        attn_mask = attn_mask[:, -stdlen-topk_num:]

        return QKk, w_head_k, w_head_v, r_head_k, attn_mask
    ...
    def forward(self, w, r, attn_mask=None, mems=None, head_mask=None, output_attentions=False, topk_num=None, remain_mem_num=None):
        ...
        if topk_num == 0:
            ...
            kK = (cat @ K.T).view(klen, bsz, self.n_head, self.d_head)
            Q_head = Q.view(self.n_head, self.d_head, self.d_model)
            QKk = torch.einsum('jbnd,ndk->jbnk', (kK, Q_head))

            QKk, w_head_k, w_head_v, r_head_k, attn_mask = self.trams(
                QKk, 
                w_head_k, 
                w_head_v, 
                r_head_k, 
                attn_mask, 
                topk_num=topk_num, 
                remain_mem_num=remain_mem_num
            )

            # Combine einsum operations
        ...
        if self.attn_type == 0:  # default
            ...
            for i, layer in enumerate(self.layers):
                ...
                layer_outputs = layer(
                    ...
                    output_attentions=output_attentions,
                    topk_num=self.args.topk_num,
                    remain_mem_num=self.args.remain_mem_num,
                )
...
class TransfoXLLMHeadModel(TransfoXLPreTrainedModel):
    ...
    def __init__(self, config, args):
        super().__init__(config)
        self.transformer = TransfoXLModel(config, args)
